    ...
    <div className="flex flex-col gap-4">
      <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
        <div className="space-y-1">
          <label className="text-xs font-medium text-muted-foreground">Palindrome Prefix</label>
          <Input value={state.prefix} onChange={e => onChange({ prefix: e.target.value })} placeholder="e.g. 123" />
        </div>
        <div className="space-y-1">
        ...
        <Select
          label="Found Status"
          value={state.found}
          onChange={val => onChange({ found: (val as 'all'|'found'|'unfound') || 'all' })}
          placeholder="All"
        >
          <SelectOption value="all">All</SelectOption>
          <SelectOption value="found">Found</SelectOption>
          <SelectOption value="unfound">Not Found</SelectOption>
        </Select>
        <Select
          label="Sort"
          value={state.sort}
